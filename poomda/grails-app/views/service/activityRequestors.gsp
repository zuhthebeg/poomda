		...
		<div class="sub-body">
			...
			<div class="active-more-buttonbox">
				...
				<div class="active-more-2-joiner">
					<g:set var="joinerList" value="${ActivityParticipants.findAllByActivityAndStatusInList(Activity.get(params.id),['REGIST','REJECT']) }" />
					<g:each in="${joinerList}" var="joiner">
						<div>
							...
							<div>
								...
								<a href="#none" onclick="requestChangeStatus(${joiner.activity.id},'${joiner.user.username }','APPROVAL')" class="red-bg">
									참가승인
								</a>
							</div>
						</div>
					</g:each>
					${!joinerList ?'신청자가 없습니다.':''}
				</div>
			...
			<div class="sponsor-navbox">
				...
				<a href="#none" onclick="requestChangeStatusByActivity(${Activity.get(params.id)},'APPROVAL')" class="sponsor-ok">
					모두 승인
				</a>
			</div>
		</div>
