<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ include file="../inc/top.jsp" %>
<%@ include file="../inc/header.jsp" %>
<link rel="stylesheet" type="text/css" href="../css/main.css">
<link rel="stylesheet" type="text/css" href="../css/service.css">
<script src="../js/sub.js"></script>
	<!-- sub-content // S -->
	<div id="sub-content" class="service">
		<div>
			<div>
				<h1>봉사/후원참여</h1>
				<p>직접 봉사활동이나 후원활동에 참여해 보세요</p>
			</div>
		</div>
		<div class="sub-body">

			<!-- active-more-buttonbox // S -->
			<div class="active-more-buttonbox">
				
				<!-- active-more-choice // S -->
				<div class="active-more-choice">
					<a href="./active_more.jsp">
						활동정보 변경
					</a>
					<a href="./active_more2.jsp" class="on">
						참여신청 관리
					</a>
				</div>
				<!-- active-more-choice // E -->
				
				<!-- active-more-2-tab // S -->
				<div class="active-more-2-tab">					
					<ul>
						<li>
							<a href="./active_more2_1.jsp" class="on">
								신청자 <span>(7)</span>
							</a>
						</li>
						<li>
							<a href="./active_more2_2.jsp">
								참가자 <span>(7)</span>
							</a>
						</li>
					</ul>
				</div>
				<!-- active-more-2-tab // E -->
				<!-- active-more-2-joiner // S -->
				<div class="active-more-2-joiner">
					<div>
						<img src="../img/common/no-image-big2.gif">
						<ul>
							<li>
								<span>닉네임</span> 간달퐁
							</li>
							<li>
								<span>연령대/성별</span> 10대/남성
							</li>
							<li>
								<span>연락처</span> 010-1111-1111
							</li>
						</ul>
						<div>
							<a href="#;">
								상세프로필
							</a>
							<a href="#;" class="modal_up">
								신청서 보기
							</a>
							<div class="question_layer">
								<h2>
									참여신청서
								</h2>
								<div>
									<h3>
										자기소개
									</h3>
									<p>
										안녕하세요. 저는 남양주시에 거주하고 있는 이환희라고 합니다. 직업은 수의사입니다.
									<p>
								</div>
								<div>
									<h3>
										참여동기 및 경험
									</h3>
									<p>
										2013년부터 유기동물 보호소에서 근무하고 있으며, 포인핸드앱을 통해서 많은 유기동물
										들이 가족을 만나는 것을 돕고 있습니다. 현재는 많은 좋은분들과 함께 동물을 돕기 위해 
										다양한 프로젝트를 진행하고 있습니다.

									<p>
								</div>
								<div>
									<h3>
										연락처
									</h3>
									<p>
										010-4727-6217
									<p>
								</div>
								<button type="button" class="send_msg">신청서 제출</button>
								<button type="button" class="close_lyr">닫기</button>
							</div>
							<a href="#;" class="red-bg">
								참가승인
							</a>
						</div>
					</div>
					<div>
						<img src="../img/common/no-image-big2.gif">
						<ul>
							<li>
								<span>닉네임</span> 간달퐁
							</li>
							<li>
								<span>연령대/성별</span> 10대/남성
							</li>
							<li>
								<span>연락처</span> 010-1111-1111
							</li>
						</ul>
						<div>
							<a href="#;">
								상세프로필
							</a>
							<a href="#;" class="modal_up">
								신청서 보기
							</a>
							<div class="question_layer">
								<h2>
									참여신청서
								</h2>
								<div>
									<h3>
										자기소개
									</h3>
									<p>
										안녕하세요. 저는 남양주시에 거주하고 있는 이환희라고 합니다. 직업은 수의사입니다.
									<p>
								</div>
								<div>
									<h3>
										참여동기 및 경험
									</h3>
									<p>
										2013년부터 유기동물 보호소에서 근무하고 있으며, 포인핸드앱을 통해서 많은 유기동물
										들이 가족을 만나는 것을 돕고 있습니다. 현재는 많은 좋은분들과 함께 동물을 돕기 위해 
										다양한 프로젝트를 진행하고 있습니다.

									<p>
								</div>
								<div>
									<h3>
										연락처
									</h3>
									<p>
										010-4727-6217
									<p>
								</div>
								<button type="button" class="send_msg">신청서 제출</button>
								<button type="button" class="close_lyr">닫기</button>
							</div>
							<a href="#;" class="red-bg">
								참가승인
							</a>
						</div>
					</div>
					<div>
						<img src="../img/common/no-image-big2.gif">
						<ul>
							<li>
								<span>닉네임</span> 간달퐁
							</li>
							<li>
								<span>연령대/성별</span> 10대/남성
							</li>
							<li>
								<span>연락처</span> 010-1111-1111
							</li>
						</ul>
						<div>
							<a href="#;">
								상세프로필
							</a>
							<a href="#;">
								신청서 보기
							</a>
							<a href="#;" class="red-bg">
								참가승인
							</a>
						</div>
					</div>
					<div>
						<img src="../img/common/no-image-big2.gif">
						<ul>
							<li>
								<span>닉네임</span> 간달퐁
							</li>
							<li>
								<span>연령대/성별</span> 10대/남성
							</li>
							<li>
								<span>연락처</span> 010-1111-1111
							</li>
						</ul>
						<div>
							<a href="#;">
								상세프로필
							</a>
							<a href="#;">
								신청서 보기
							</a>
							<a href="#;" class="red-bg">
								참가승인
							</a>
						</div>
					</div>
				</div>
				<!-- active-more-2-joiner // E -->

			</div>
			<!-- main-service-list Program box // E -->

			<div class="sponsor-navbox">
				<a href="#;" class="sponsor-cancle">
					취 소
				</a>
				<a href="#;" class="sponsor-ok">
					완 료
				</a>
			</div>
		</div>
	</div>
	<!-- sub-content // E -->
<%@ include file="../inc/footer.jsp" %>